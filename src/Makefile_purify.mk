...
$(BUILD_DIR)/%.o : %.c
	purify -chain-length=10 -follow-child-processes=yes gcc $(CXXFLAGS) $(CFLAGS) $(INCLUDES) -MD -c $< -o $@
...
$(BIN_DIR)/pcapdistributer: $(COMMON_OBJECTS) $(BUILD_DIR)/filterprograms/pcapdistributer.o
	purify -chain-length=10 -follow-child-processes=yes gcc ${CXXFLAGS} ${CFLAGS} $(LDLIBS) $(COMMON_OBJECTS) $(BUILD_DIR)/filterprograms/pcapdistributer.o -o $@

$(BIN_DIR)/pcapmerger: $(COMMON_OBJECTS) $(BUILD_DIR)/filterprograms/pcapmerger.o
	purify -chain-length=10 -follow-child-processes=yes gcc ${CXXFLAGS} ${CFLAGS} $(LDLIBS) $(COMMON_OBJECTS) $(BUILD_DIR)/filterprograms/pcapmerger.o -o $@

$(BIN_DIR)/decodegtp: $(COMMON_OBJECTS) $(BUILD_DIR)/filterprograms/decodegtp.o
        purify -chain-length=10 -follow-child-processes=yes gcc ${CXXFLAGS} ${CFLAGS} $(LDLIBS) $(COMMON_OBJECTS) $(BUILD_DIR)/filterprograms/decodegtp.o -o $@
